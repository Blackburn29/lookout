...
```bash
lookout --partition nopersist
```

Alternatively, you can use a file called `config.json` with the configuration options. This file needs to be located in `~/.config/lookout/config.json`.
For Snap installations, the file is located in `~/snap/lookout/current/.config/lookout/config.json` and for Flatpak it is located in `~/.var/app/com.github.blackburn29.outlook_for_linux/config/lookout/config.json`.
