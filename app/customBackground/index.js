...
class CustomBackground {
    ...
    beforeRequestHandlerRedirectUrl(details) {
        ...
        if (details.url.startsWith('https://statics.outlook.cdn.office.net/lookout/custom-bg/')) {
            const reqUrl = details.url.replace('https://statics.outlook.cdn.office.net/lookout/custom-bg/', '');
...
function setPath(cfg) {
    if (!cfg.src.startsWith('/lookout/custom-bg/')) {
        cfg.src = httpHelper.joinURLs('/lookout/custom-bg/', cfg.src);
    }

    if (!cfg.thumb_src.startsWith('/lookout/custom-bg/')) {
        cfg.thumb_src = httpHelper.joinURLs('/lookout/custom-bg/', cfg.thumb_src);
    }
}
